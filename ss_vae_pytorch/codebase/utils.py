...
def log_normal(x, m, v):
    ...
    # the last dimension
    ################################################################################
    log_gauss_prob = -0.5 * (torch.log(2 * v * np.pi) + (x - m).pow(2) / v )
    log_prob = log_gauss_prob.sum(dim = -1)
...
def log_normal_mixture(z, m, v):
    ...
    ################################################################################
    z = z.unsqueeze(1)
    log_normal_prob = log_normal(z, m, v)
    log_prob = log_mean_exp(log_normal_prob, dim = 1)
    ################################################################################
    # End of code modification
    ################################################################################
    return log_prob
