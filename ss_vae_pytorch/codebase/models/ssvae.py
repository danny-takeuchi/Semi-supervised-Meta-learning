...
class SSVAE(nn.Module):
    ...
    def negative_elbo_bound(self, x):
        ...
        mu, var = self.enc.encode(x, y)
        z = ut.sample_gaussian(mu, var)
        decoder_out = self.dec.decode(z, y)

        # y_prior = torch.log(1 / torch.ones_like(y_logits) * 10)

        # kl_y = ut.kl_cat(y_prob, y_logprob, y_prior)
        kl_y = ut.kl_cat(y_prob, y_logprob, np.log(1.0 / self.y_dim))
        kl_z = ut.kl_normal(mu, var, self.z_prior_m, self.z_prior_v)
        rec = -ut.log_bernoulli_with_logits(x, decoder_out)

        kl_z = y_prob.transpose(0, 1) * kl_z.view(self.y_dim, -1)
        kl_z = kl_z.sum(dim = 0)

        rec = y_prob.transpose(0, 1) * rec.view(self.y_dim, -1)
        rec = rec.sum(dim = 0)
